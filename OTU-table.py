...
otu_list = pd.read_csv(args.otulist, sep="\t", index_col=1, header=None, names=["OTU_ID"])
# Group the ASV counts by their corresponding OTU
otu_counts = asv_counts.groupby(otu_list["OTU_ID"]).sum()
# Write the OTU count table to a file
otu_counts.to_csv(args.output, sep="\t")
